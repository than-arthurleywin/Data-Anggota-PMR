...
<div class="beranda -z-50">
    <h3 class="text-3xl mb-4">Kelola Angkatan</h3>
    <div class="flex">
...
<div class="mt-2 flex justify-between items-center">
    <p class="text-xs text-gray-500">
        Menampilkan {{ $angkatan->firstItem() }} - {{ $angkatan->lastItem() }} dari {{ $angkatan->total() }} data
    </p>
    <div class="flex space-x-2">
        @if ($angkatan->onFirstPage())
            <button class="px-4 py-2 bg-gray-300 text-gray-700 rounded-md cursor-not-allowed">Previous</button>
        @else
            <a href="{{ $angkatan->previousPageUrl() }}" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-500">Previous</a>
        @endif

        @if ($angkatan->hasMorePages())
            <a href="{{ $angkatan->nextPageUrl() }}" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-500">Next</a>
        @else
